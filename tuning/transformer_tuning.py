...
def objective(trial, num_patients=350, batch_size=64, num_epochs=300):
    ...
    # Hyperparameters
    length = raw_features_train.shape[1]
    d_init = raw_features_train.shape[2]
    d_model = trial.suggest_int("d_model", 10, 50, step=10)
...
def transformer_tuning():
    study = optuna.create_study(direction="minimize", study_name="Transformer tuning all params")
    study.optimize(objective, n_trials=50, timeout=100000)  # Timeout is in seconds
